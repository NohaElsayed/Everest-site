...
class SystemController extends Controller
{
    ...
    public function set_delivery_method(Request $request)
    {
        if ($request['cart_group_id'] == 'all_cart_group') {
            foreach (CartManager::get_cart_group_ids() as $group_id) {
                $request['cart_group_id'] = $group_id;
                self::insert_into_cart_delivery($request);
            }
        } else {
            self::insert_into_cart_delivery($request);
        }
    ...
    public static function insert_into_cart_delivery($request)
    {
        $delivery = CartDelivery::where(['cart_group_id' => $request['cart_group_id']])->first();
        if (isset($delivery) == false) {
            $delivery = new CartDelivery();
        }
        $delivery['cart_group_id'] = $request['cart_group_id'];
        $delivery['delivery_method_id'] = $request['id'];
        $delivery->save();

        $shipping = CartShipping::where(['cart_group_id' => $request['cart_group_id']])->first();
        $deliveryMethod = DeliveryMethod::where(['id' => $request['id']])->first();

        if (isset($shipping)) 
        {
            if($deliveryMethod->slug == "pick up")
            {
               $shipping->delete();
            }
        }
    ...
    public function choose_store_address(Request $request)
    {
    
        $store = [];
        parse_str($request->store, $store);
       
        session()->put('store_ids', $store);  
       
        return response()->json([], 200);
    }

    public function choose_shippingstore_address(Request $request)
    {
        $shipping = [];
        $billing = [];
        $store = [];
