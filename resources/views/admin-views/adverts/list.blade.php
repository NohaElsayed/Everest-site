...
@section('content')
    <div class="content container-fluid">
        ...
        <div class="d-sm-flex align-items-center justify-content-between mb-2">
            <h1 class="h3 mb-0 text-black-50">{{\App\CPU\translate('Advertisement_list')}} <span style="color: rgb(252, 59, 10);">({{ $br->total() }})</span></h1>
        </div>
                        ...
                        <form action="{{ url()->current() }}" method="GET">
                            <div class="input-group input-group-merge input-group-flush">
                                ...
                                <input id="datatableSearch_" type="search" name="search" class="form-control"
                                    placeholder="{{ \App\CPU\translate('Search')}} {{ \App\CPU\translate('Advertisements')}}" aria-label="Search orders" value="{{ $search }}" required>
                                <button type="submit" class="btn btn-primary">{{ \App\CPU\translate('Search')}}</button>
                            </div>
                        </form>
                        <!-- End Search -->

{{--                        <a href="{{ route('admin.adverts.export') }}" class="btn btn-success">--}}
                            ...
                            <table style="text-align: {{Session::get('direction') === "rtl" ? 'right' : 'left'}};"
                                ...
                                <tr>
                                    <th scope="col" style="width: 100px">
                                        {{ \App\CPU\translate('Advertisement')}} {{ \App\CPU\translate('ID')}}
                                    </th>
...
{{--                                        <td>{{ $b['brandAllProducts']->sum('order_details_count') }}</td>--}}
                                        ...
                                        <td>
                                            <a class="btn btn-primary btn-sm" title="{{ \App\CPU\translate('Edit')}}"
                                               href="{{route('admin.adverts.update',[$b['id']])}}">
                                                <i class="tio-edit"></i>
                                            </a>
...
@push('script')
    <script>
        $(document).on('click', '.delete', function () {
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.ajax({
                        url: "{{route('admin.adverts.delete')}}",
                        method: 'POST',
                        data: {id: id},
                        success: function () {
                            toastr.success('{{ \App\CPU\translate('Advertisement_deleted_successfully')}}');
                            location.reload();
                        }
                    });
                }
        ...
        $(document).on('change', '.status', function () {
            ...
            $.ajax({
                url: "{{route('admin.adverts.status-update')}}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
