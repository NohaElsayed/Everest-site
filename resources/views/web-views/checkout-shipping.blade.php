...
@php($cartDeliveries = \App\Model\CartDelivery::whereIn('cart_group_id', $cart_group_ids)->pluck('delivery_method_id')->toArray())
@php($x=0)
@if(count(array_unique($cartDeliveries)) === 1)
   @php($deliveryMethod = \App\Model\DeliveryMethod::where('id',$cartDeliveries[0])->first())
        @php($x= $deliveryMethod->slug == 'pick up' ? 1 : 2)
@else
@php($x=3)
@endif
       ...
       <div class="row">
            <div class="col-md-12 mb-5 pt-5">
                <div class="feature_header">
                    <span>{{ \App\CPU\translate('shops')}}</span>
                </div>
            </div>
            <section class="col-lg-8">
                ...
                <!-- Shipping methods table-->
                    <h2 class="h4 pb-3 mb-2 mt-5">{{ \App\CPU\translate('choose_shop_address')}}</h2>
                    @php($sellerIds = \App\Model\Cart::whereIn('cart_group_id', $cart_group_ids)->groupBy('cart_group_id')->pluck('seller_id')->toArray())
                    ...
                    <form method="post" action="" id="store-form">
                        ...
                        <div class="card-body" style="padding: 0!important;">
                            <ul class="list-group">
                              ...
                              {{ \App\CPU\translate('store')}}{{'#'.($index+1)}}
                              @php($shops = \App\Model\Shop::where('seller_id',$sellerId)->get())
                                @foreach($shops as $key=>$shop)
                                    <li class="list-group-item mb-2 mt-2"
                                        style="cursor: pointer;background: rgba(245,245,245,0.51)"
                                        onclick="$('#sl-{{$shop['id']}}').prop( 'checked', true )">
                                        <input type="radio" name="{{$sellerId}}"
                                               id="sl-{{$shop['id']}}"
                                               value="{{$shop['id']}}" {{$key==0?'checked':''}}>
                                        <span class="checkmark"
                                              style="margin-{{Session::get('direction') === "rtl" ? 'left' : 'right'}}: 10px"></span>
                                        <label class="badge"
                                               style="background: {{$web_config['primary_color']}}; color:white !important;">{{$shop['name']}}</label>
                                        <small>
                                            <i class="fa fa-phone"></i> {{$shop['contact']}}
                                        </small>
                                        <hr>
                                        <span>{{ \App\CPU\translate('contact_shop_name')}}: {{$shop['name']}}</span><br>
                                        <span>{{ \App\CPU\translate('address')}} : {{$shop['address']}} .</span>
                                    </li>
                                @endforeach
                               @endforeach
                            </ul>
      ...
      @elseif($x == 2)   
        <div class="row">
            ...
            <section class="col-lg-8">
                ...
                <div class="checkout_details mt-3">
                @php($delivery=0)
                    @foreach($cartDeliveries as $cartIndex => $cartDelivery)
                      @php($deliveryCart = \App\Model\DeliveryMethod::where('id',$cartDelivery)->first())
                        @if($deliveryCart->slug == 'pick up')
                               <!-- Shipping methods table-->
                    <h2 class="h4 pb-3 mb-2 mt-5">{{ \App\CPU\translate('choose_shop_address')}}</h2>
                    @php($cart_group_id = \App\Model\CartDelivery::where('delivery_method_id',$cartDelivery)->first()->cart_group_id)

                    @php($sellerId = \App\Model\Cart::where('cart_group_id', $cart_group_id)->first()->seller_id)
                    ...
                    <form method="post" action="" id="store-form">
                        ...
                        <div class="card-body" style="padding: 0!important;">
                            <ul class="list-group">
                              ...
                              {{ \App\CPU\translate('store')}}{{'#'.($cartIndex+1)}}
                              @php($shops = \App\Model\Shop::where('seller_id',$sellerId)->get())
                                @foreach($shops as $key=>$shop)
                                    <li class="list-group-item mb-2 mt-2"
                                        style="cursor: pointer;background: rgba(245,245,245,0.51)"
                ...
                @endif        
                    ...
                    <div class="row">
                        ...
                        @if($x == 1)
                        <div class="col-6">
                            <a class="btn btn-primary btn-block" href="javascript:" onclick="store_to_next()">
                       ...
                       @elseif($x == 2)
                       <div class="col-6">
                            <a class="btn btn-primary btn-block" href="javascript:" onclick="address_to_next()">
                                <span class="d-none d-sm-inline">{{ \App\CPU\translate('proceed_payment')}}</span>
                                <span class="d-inline d-sm-none">{{ \App\CPU\translate('Next')}}</span>
                                <i class="czi-arrow-{{Session::get('direction') === "rtl" ? 'left' : 'right'}} mt-sm-0 mx-1"></i>
                            </a>
...
@push('script')
    ...
    <script>
        function initAutocomplete() {
            var myLatLng = {
                lat: {{isset($default_location) ?$default_location['lat']:'-33.8688'}},
                lng: {{isset($default_location) ?$default_location['lng']:'151.2195'}}
            };
...
$.post({
    url: '{{route('customer.choose-store-address')}}',
    // dataType: 'json',
    data: {
        store: $('#store-form').serialize(),
    },
    beforeSend: function () {
        $('#loading').show();
    },
...
$.post({
    url: '{{route('customer.choose-shippingstore-address')}}',
    // dataType: 'json',
    data: {
        shipping: $('#address-form').serialize(),
        billing: $('#billing-address-form').serialize(),
